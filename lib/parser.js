...
export function parseProxyLinks(input) {
  ...
  for (let line of lines) {
    ...
    try {
      ...
      while (currentLine && !proxy) {
        if (currentLine.startsWith('hysteria2://') || currentLine.startsWith('hy2://')) proxy = parseHysteria2(currentLine)
        else if (currentLine.startsWith('anytls://')) proxy = parseAnyTLS(currentLine)
        else if (currentLine.startsWith('vless://')) proxy = parseVless(currentLine)
        else if (currentLine.startsWith('trojan://')) proxy = parseTrojan(currentLine)
        else if (currentLine.startsWith('vmess://')) proxy = parseVmess(currentLine)
        else if (currentLine.startsWith('ss://')) proxy = parseSS(currentLine)
        else if (currentLine.startsWith('tuic://')) proxy = parseTuic(currentLine)
        else if (!isDecoded) {
          isDecoded = true
          try {
            currentLine = decodeBase64(currentLine).trim()
          } catch {
            break // 防御非法字符导致的解包失败
          }
        } else {
          break // 已尝试解码一次，仍不匹配则退出
        }
      }
...
// 兼容 Node.js 与浏览器的安全 Base64 解码函数
function decodeBase64(str) {
  if (!str) return ''

  // 1. 替换 URL-Safe 字符，并补齐末尾缺失的 '='
  ...
  // 3. 浏览器环境（使用 TextDecoder 防止中文/UTF-8 节点名乱码）
